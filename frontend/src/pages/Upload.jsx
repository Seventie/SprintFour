    ...
    <PageWrapper>
      <main className="flex-1 flex flex-col items-center justify-center px-4 w-full max-w-3xl mt-20 mb-24 animate-fade-in-up">
        ...
        <p className="text-lg text-brand-gray mb-10 max-w-xl text-center">
          Upload documents (TXT, DOCX, PDF) to begin redaction analysis.
        </p>
        ...
        >
          ...
          <label 
            htmlFor="file-upload"
            className="px-8 py-3.5 bg-white text-[#121212] border border-[#e5e7eb] rounded-full font-semibold text-sm hover:bg-gray-50 transition-all duration-300 hover:scale-105 cursor-pointer shadow-sm"
          >
            Browse Files
          </label>
        </div>
            ...
            <ul className="space-y-3 mb-8 max-w-xl mx-auto">
              {files.map((file, idx) => (
                <li key={idx} className="flex items-center gap-4 p-4 bg-white rounded-2xl shadow-sm border border-[#e5e7eb]">
                  ...
                  <span className="text-xs text-gray-400 font-medium">{(file.size / 1024).toFixed(1)} KB</span>
                </li>
              ))}
            </ul>
